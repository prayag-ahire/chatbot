...
// ---------------------------------------------
// BUILD HUMAN-READABLE WEEKLY SCHEDULE
// ---------------------------------------------
...
// ---------------------------------------------
// CHECK IF ORDER MATCHES SCHEDULE
// ---------------------------------------------
const isOrderInSchedule = (
  orderDateStr: string,
  orderTimeStr: string,
  ...
  const map = [
    { s: schedule.start_sunday, e: schedule.end_sunday },
    { s: schedule.start_monday, e: schedule.end_monday },
    { s: schedule.start_tuesday, e: schedule.end_tuesday },
    { s: schedule.start_wednesday, e: schedule.end_wednesday },
    { s: schedule.start_thursday, e: schedule.end_thursday },
    { s: schedule.start_friday, e: schedule.end_friday },
    { s: schedule.start_saturday, e: schedule.end_saturday },
  ];

  const slot = map[day];
  if (!slot.s || !slot.e) return false;

  return orderTimeStr >= slot.s && orderTimeStr <= slot.e;
};
...
// ====================================================================================
// FETCH WORKER DATA (MAIN FUNCTION)
// ====================================================================================
export const fetchWorkerData = async (workerId: number): Promise<WorkerContext | null> => {
  try {
    ...
    const reviewAnalytics = {
      ...
      // Fixed: Replace .at(-1) with standard array index access
    ...
    const week_summary = buildWeeklySummary(weekSchedule);
    ...
    // --------------------------------------------------
    // 8. MONTHLY HISTORY ANALYTICS
    // --------------------------------------------------
    const monthlyHistoryMap: Record<string, MonthlyMetric> = {};
    const chargesPerVisit = Number(workerData.charges_pervisit) || 0;

    const today = new Date();
    // Use ISO string for map keys to ensure consistency
    for (let i = 0; i < 12; i++) {
    ...
    orders.forEach(o => {
      ...
      if (!monthlyHistoryMap[key]) {
        // Create it if it's older than 12 months but exists in data
        monthlyHistoryMap[key] = {
          month_name: `${d.toLocaleString("en-US", { month: "long" })} ${d.getFullYear()}`,
          year: d.getFullYear(),
          month: d.getMonth(),
    ...
    // Current month is index 0 if sorted DESC by date
    const currentMonth = monthlyHistory[0];

    // --------------------------------------------------
    // 9. SYSTEM-WIDE WORKER DATA (for comparisons)
    // --------------------------------------------------
    const { data: allWorkers } = await supabase
      .from("worker")
      .select("id, rating, charges_perhour, profession, gender") as any;

    const { data: allOrders } = await supabase
      .from("workerorder")
      .select("workerid, order_status, date, time") as any;

    const { data: allLocations } = await supabase
      .from("location")
      .select("latitude, longitude, workersettings!inner(workerid)") as any;

    const { data: allWeekSchedules } = await supabase
      .from("weekschedule")
      .select("*") as any;

    // ================================================
    // 10. RADIUS-BASED PEER ANALYTICS
    // ================================================
    ...
    if (locationData && allLocations && allWorkers) {
      ...
      const myProf = normalizeProf(workerData.profession);
      const myGender = normalizeGender(workerData.gender);
    ...
    // ================================================
    // 11. SYSTEM BENCHMARKS & PERFORMANCE SCORES
    // ================================================
    ...
    if (allWorkers && allWorkers.length > 0) {
      ...
      // ----------------------------------------
      // Profession Peer Stats
      // ----------------------------------------
      // "Peers" includes EVERYONE in the profession, including self, for stats
      ...
      topProfessions = Array.from(professionMap.entries())
        .map(([p, data]) => ({
          profession: p, // Use normalized key or original? Normalized is safer for aggregation
      ...
      if (allLocations) {
        // Count per coordinate cluster (rounded)
        allLocations.forEach((loc: any) => {
