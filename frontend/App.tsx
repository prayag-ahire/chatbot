...
export default function App() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (!isHealthy) {
          console.warn('Backend API not connected - make sure server is running on port 3001');
        }
      } catch (error) {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      if (!apiConnected) {
        throw new Error('Backend API is not connected. Please start the server on port 3001.');
      }

      const responseText = await sendChatMessage(userMsg.content, workerContext);
          ...
          <div className="max-w-4xl mx-auto relative">
            <form onSubmit={handleSendMessage} className="flex items-center space-x-2">
              ...
              <button
                ...
                className="p-3 text-white bg-indigo-600 rounded-full hover:bg-indigo-700 disabled:bg-gray-400 transition-colors shadow-sm"
              >
                <Send size={18} />
              </button>
            </form>
